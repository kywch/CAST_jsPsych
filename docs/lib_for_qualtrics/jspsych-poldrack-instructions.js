    ...
    function show_current_page() {
      if (trial.show_clickable_nav) {
        ...
        if (trial.pages[current_page].slice(-6) == '</div>') {
          jQuery(display_element).html(trial.pages[current_page].slice(0, -6) + nav_html + '</div>')
        } else {
          jQuery(display_element).html(trial.pages[current_page] + nav_html + '</div>')
        }
